...
#include "../include/cubic_example.h"
#include <vector>
#include <chrono>
...
int main(int argc, char** argv)
{
    ...
    path.push_back(Vector3d(0.5,0.25,0.0));
    ...
    path.push_back(Vector3d(4.0,1.0,0.0));
    ...
    // std::vector<double> ex_pathx(sizeof(ex.m_coeffs)/sizeof(*ex.m_coeffs));
    // std::vector<double> ex_pathy(sizeof(ex.m_coeffs)/sizeof(*ex.m_coeffs));
    ...
    std::vector<double> c_pathat(c_spline.GetAccelerationProfile().size());
    ...
    for(int i=0;i<c_pathx.size();++i)
    {
        ...
        c_pathat[i] = sqrt(c_pathax[i]*c_pathax[i] + c_pathay[i]*c_pathay[i]);
        c_pathc[i] = c_spline.GetCurvatureProfile()[i];
        ti[i] = i/(double)divisions;
        // std::cout << c_pathx[i] << " , " << c_pathy[i]<< std::endl;
    }
    
    // Get the length of a spline between two knots
    for(int i=0;i<c_spline.GetPoints().size()-1;i++)
    {
        std::cout << c_spline.GetSplineLengths()[i] << std::endl;
    }
    ...
    // matplotlibcpp::plot(ex_pathx, ex_pathy, "b-");   // show plots
    matplotlibcpp::figure(2);
    matplotlibcpp::title("X-Velocity");
    ...
    matplotlibcpp::plot(ti, c_pathat, "r-");  
